# HN-Extension-Pathfindermods-manual
关于使用MOD来制作Hacknet扩展的手册
你必须安装[Pathfinder](https://github.com/Arkhist/Hacknet-Pathfinder)才能正常游玩拥有mod的扩展

> [!WARNING]\
> 在此说明，由于仓库作者个人时间有限，目前该项目不定期更新。我对此表示诚挚的歉意。希望能得到您的理解与支持。

## 前情提要

-本手册强烈推荐在已有一定原版扩展制作基础上进行学习 

-本手册需要了解 **扩展制作指南** 等的有关内容(*已列举*)

-本手册对于mod的介绍依据于Github上的项目。

-本手册的部分内容可能会因MOD的更新而不适用，所以此手册特别标明介绍的MOD的目前版本。 

-本手册的制作是一个长期的过程，如有不对之处，望指出，便于改正。谢谢！

此手册作者 **ZQG** 和 **WnCry**

## 开始阅读

[开始阅读](./start.md)

## 可以参考的内容

- [*Hacknet Extensions Guide (Official)*](https://steamcommunity.com/sharedfiles/filedetails/?id=914587661)

- *IntroExtension*

- *《Hacknet 扩展制作指南 – 入门》* by Pitiedwzr & Err0r233

- *《Hacknet 主机编辑指南》* by Cyl18

- [*Hacknet 扩展教程(FBIK)*](https://github.com/FBIKdot/Hacknet-Extension-Tutorial/)

- [*Hacknet 扩展教程(Vert)*](https://github.com/maix2012/Hacknet-Extension-Tutorial-Vert/)

- 开发中的标准：[hnxml](https://hnxml.js.org)

- 你也可以来到Q群**940056115**询问
